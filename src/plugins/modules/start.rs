...
use grammers_client::{Client, InputMessage, types};
...
async fn start_message(_client: Client, data: Data) -> Result {
    ...
    let input = InputMessage::html(locale.get("plugins.start.start").format(&[user.name(), me.first_name()])).reply_markup(&markup);

    match message.chat() {
        types::Chat::Group(_) => {
            let markup = utils::make_keyboard(vec![
                &[(&locale.get("buttons.pm"), &"https://t.me/{}/?start".format(&[me.username().unwrap()]), "url")]
            ]);
            message.reply(InputMessage::html(locale.get("plugins.start.start_group").format(&[user.name(), me.first_name()])).reply_markup(&markup)).await?;
